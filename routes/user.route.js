...
const { HandleNullString } = require('../middlewares');

router.route('/')
.get(GetAll)                // GET ALL USERS
.post(upload.single('image'), HandleNullString, Create)               // CREATE USER

router.route('/:id')
.get(Get)                   // GET USER AGAINST ID
.put(upload.single('image'), HandleNullString, Update)                // UPDATE USER
